...
import com.brys.dev.api.SpotifyWeb as SpotifyWeb1
...
fun logCacheSize() {
    ...
    println(c.brightBlue(" Playlist Size: ${SpotifyWeb1.playlistTracks.size}"))
    println(c.brightBlue(" Track Size: ${SpotifyWeb1.tracks.size}"))
    println(c.brightBlue(" Album Size: ${SpotifyWeb1.albumTracksCache.size}"))
    println(c.brightRed("└─────────────────────────────┘"))
    printAndRun(false)
}
fun logResetCache() {
    println(c.brightRed("┌─────────Cache Reset──────────┐"))
    println(c.brightBlue(" All ${SpotifyWeb1.playlistTracks.size.plus(SpotifyWeb1.tracks.size)} items have been reset"))
    println(c.brightRed("└─────────────────────────────┘"))
    SpotifyWeb1.playlistTracks.clear()
    SpotifyWeb1.tracks.clear()
    printAndRun(false)
}
fun logMemory() {
